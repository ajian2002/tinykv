...
const (
	...
	//我们在 DB 中保存两种类型的区域数据，用于 raft 和其他元数据。 store开始的时候，我们要遍历所有的region元数据来构造peer，不需要走大量的raft数据，所以我们用不同的前缀将它们分开。
...
//RaftLogIndex 从 raft_log_key 生成的 raft log key 中获取日志索引。
